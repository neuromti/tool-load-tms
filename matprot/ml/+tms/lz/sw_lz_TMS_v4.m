...
classdef sw_lz_TMS_v4 < handle
    
    properties
        marker_pin      = [1, 5, 9]; % Pin to only add marker to buffer
        stim_pin        = 3;      % Pin to stimulate and add marker to buffer and localite
        stim_pin_woLoc  = 2;      % Pin to stimulate and add marker to buffer but not localite
...
%                             }, 'FontSize', 14);
        ...
        function get_RMT(obj)
            ...
            % frequency method
            % Choose the lowest intensity eliciting MEPs >50?V ptp
            ...
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            % obj.time = 'pre';
            if exist([obj.directories.mt 'RMT_' obj.time '.mat'],'file') == 2
                obj.time = input('File exists already. Redo Time: ','s');
            end
            run = 1;
            finish = 1;
            exit_run = 1; 
            ...
            save([obj.directories.mt 'RMT_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
...
%             keyboard
        ...
        function gep_opt_ipi_ipsi(obj, feedback_muscle)
            ...
            % obj.time = 'pre';
            if exist([obj.directories.imep 'opt_ipi_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.imep 'opt_ipi_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            end
...
%                                         pause(1)
                                        ...
                                        if(~isempty(obj.dataEEGEMG))
                                            ...
                                            if(stimTime > 40 && ~isnan(stimTime))
                                                tmp_m                               = obj.tms_settings.targetmuscle;
                                                obj.tms_settings.targetmuscle       = feedback_muscle;
                                                data = obj.filterData(obj,obj.tms_settings.addNotchFilt);
                                                obj.tms_settings.targetmuscle       = tmp_m;
            ...
            save([obj.directories.imep 'pp_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function get_ipi(obj)
            ...
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            % obj.time = 'pre';
            if exist([obj.directories.pp 'ipi_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.pp 'ipi_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            save([obj.directories.pp 'ipi_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function get_idi(obj)
            ...
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            % obj.time = 'pre';
            if exist([obj.directories.pp 'idi_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.pp 'idi_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            disp('Saving...')
            if ~isdir(obj.directories.pp); mkdir(obj.directories.pp); end
            save([obj.directories.pp 'idi_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function get_MVC(obj)
            ...
            % steps
            obj.mvc = []; obj.mvc.step = []; %Clear previous data
            obj.check_buffer() %Check if buffer is still recording
            obj.objMag.turnOnOff('off');
            obj.objMag.flushData();
            obj.dataEEGEMG = [];
%             obj.time = input('Enter filename for saving (e.g. EDC)','s');
            if exist([obj.directories.mvc 'MVC_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.mvc 'MVC_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            pre_contr = nan(1,10);
            % Preperation time I
            obj.PTB.displayMessage('Prepare');
%             play(obj.audio.prep);
            pause(2)
...
%             play(obj.audio.prep);
            ...
            save([obj.directories.mvc 'MVC_' obj.time], 'obj', '-v7.3')
            fprintf('Finished\nMVC mean is %.3f\n', obj.mvc.mean)
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
        end
        ...
        function get_AMT(obj)
            ...
            % frequency method
            % Choose the lowest intensity eliciting MEPs >200?V ptp
            % amplitude in at least 5 of 10 trials
            % First, stimulus intensity is gradually increased in steps of 5% MSO until TMS consistently evokes MEPs
            % Thereafter, stimulus intensity is gradually lowered in steps of 1% MSO until less than 5/10. 
            ...
            obj.objMag.flushData();
            obj.dataEEGEMG = []; % Clear data matrix
            if exist([obj.directories.mt 'AMT_' obj.time '.mat'],'file') == 2
                obj.time = input('Save as: ','s');
                if exist([obj.directories.mt 'AMT_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Save as: ','s');
                end
            end
            run = 1;
            finish = 1;
...
%                                                 while toc(isiTic) < (6.25-3.75)*rand(1)+3.75, end %500ms for MagVen to change int
                                            ...
                                            if(~isempty(obj.dataEEGEMG))
                                                ...
                                                if(stimTime > 40 && ~isnan(stimTime))
                                                    data = obj.filterData(obj, obj.tms_settings.addNotchFilt);
                                                    obj.get_AMT_MEP_inf(obj,data,stimTime,run,rep, obj.tms_settings.threshold_amt);
...
%                                             tmp = sum(double(obj.MEP.isMEP(run,:)));
                                            if tmp >= 5; exit_run = 0; break
                                            elseif (rep-tmp) >= 6; exit_run = 0; break
                                            end
                                        end
...
%                                 keyboard
            ...
            disp('Saving...')
            if ~isdir(obj.directories.mt); mkdir(obj.directories.mt); end
            save([obj.directories.mt 'AMT_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        
        function get_opt_ipi_ipsi(obj, feedback_muscle)
            % Function to determine if iMEP is detectable with different IPIs
            % Pseudo randomized application of 10 stimuli at 100% MSO with set % of MVC
            % IPIs are set by tms_settings.ipi_imep
            obj.check_buffer() %Check if buffer is still recording
...
%                                         pause(1)
        ...
        function lat_current(obj)
            ...
            obj.objMag.flushData();
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            if exist([obj.directories.lat_current 'amt_lat_' obj.time '.mat'],'file') == 2
                obj.time = input('File exists already. Redo Time: ','s');
            end
            obj.objMag.flushData();
            finish  = 1;
            run     = 1;
            set_back_int = 0;
            obj.tms_settings.coil_orientation = {'AP (315?)' 'PA (45?)' 'LM (90?)'};
            obj.tms_settings.coil_orientation = obj.tms_settings.coil_orientation(randperm(numel(obj.tms_settings.coil_orientation))); %random order of coil orientation
            while finish
                if(obj.objMag.spObject.BytesAvailable~=0)
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk,'stimulate')
                                %Start stimulation protocol
                                while finish
                                    if contains(obj.tms_settings.coil_orientation{run},'LM')
                                        repetitions = 10;
                                        amp = floor(1.5*obj.tms_settings.amt_LM);
                                        if amp < 50
                                            amp = 50;
                                        end
                                    elseif contains(obj.tms_settings.coil_orientation{run},'AP')
                                        repetitions = 20;
                                        amp = floor(1.1*obj.tms_settings.amt_AP);
                                    elseif contains(obj.tms_settings.coil_orientation{run},'PA')
                                        repetitions = 20;
                                        amp = floor(1.1*obj.tms_settings.amt_PA);
                                    end
                                    
                                    fprintf('Choose coil orientation %s\n', obj.tms_settings.coil_orientation{run})
%                                     input('Press Enter to Continue')
disp('Stimulate when ready')
                                    pause_till_fbk = 1;
                                    while pause_till_fbk
                                        try
                                        mag_fbk = obj.check_mag(obj);
                                        if strcmpi(mag_fbk,'stimulate')
                                            pause_till_fbk = 0;
...
%                                             disp('Try Stimulate again')
                                    ...
                                    if run >= numel(obj.tms_settings.coil_orientation)
                                        finish = 0;
                                    else
                                        obj.pauseFunc(obj,0,amp);
                                    end
            ...
            save([obj.directories.lat_current 'amt_lat_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function iMEP_test(obj,feedback_muscle)
                                    ...
                                    for rep = 1:10
                                        ...
                                        if(~isempty(obj.dataEEGEMG))
                                            ...
                                            if(stimTime > 40 && ~isnan(stimTime))
                                                ...
                                                [pre(run,rep,:), post(run,rep,:)] = obj.getiMEPinf(obj,data,stimTime,run,rep);
                                                obj.plotiMEP(obj, data, stimTime, rep, h1)
                                                obj.objLoc.sendMEP(obj.MEP.peakToPeak(run,rep),obj.MEP.peakToPeak(run,rep), obj.MEP.duration(run,rep));
                                            else
                                                obj.MEP.iMEP(run,rep)  = 0;
            ...
            disp('Saved')
            if strcmpi(input('Try average analysis?:  y/n \n', 's'), 'y')
                data = nanmean(post(1,:,:),2);
                baseline = nanmean(pre(1,:,:),2);
                [is_iMEP, ptp]  = obj.get_iMEP_average(obj,data,baseline);
                if is_iMEP
                    fprintf('iMEP detected with a delta MEP of %.3f\n', ptp)
                else
                    disp('NO iMEP detected')
                end
            end
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.changeAmp(0,0)
        ...
        function restEEG(obj, duration)
            ...
            data_tmp = [];
            % obj.time = 'pre';
            if exist([obj.directories.EEG 'restingEEG_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.EEG 'restingEEG_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            save([obj.directories.EEG 'restingEEG_' obj.time], 'obj', '-v7.3')
        ...
        function CMC(obj, duration)
            data_tmp = [];
            % obj.time = 'pre';
            if exist([obj.directories.EEG 'CMC_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.EEG 'CMC_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            obj.dataEEGEMG = data_tmp; % read out data
            if ~isdir(obj.directories.EEG); mkdir(obj.directories.EEG); end
            save([obj.directories.EEG 'CMC_' obj.time], 'obj', '-v7.3')
        ...
        function get_IO(obj)
            ...
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            % obj.time = 'pre';
            if exist([obj.directories.io 'io_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.io 'io_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
                        ...
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                while finish
                                    ...
                                    amp = obj.tms_settings.io_int(1,int);
                                    ...
                                    for rep = 1:10
                                        ...
                                        display(['Rep: ' num2str(rep)])
                                        %stimulate with specified intensity if target muscle at rest and get buffer data
                                        [tmp,isiTic] = obj.stimTMS_rest(obj);
%                                         [tmp,isiTic] = obj.stimTMS_rest(obj,obj.tms_settings.io_int(1,int));
                                    ...
                                    obj.objMag.changeAmp(0,0);    % change MagVen to 0 MSO
...
%                                         finish = 0;
            ...
            save([obj.directories.io 'io_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function get_CSE(obj)
            ...
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            % obj.time = 'pre';
            if exist([obj.directories.mep 'mep_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.mep 'mep_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
                        ...
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                while finish
                                    ...
                                    obj.objMag.changeAmp(0,0);    % change MagVen back to 0%MSO
            ...
            save([obj.directories.mep 'mep_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        
        function get_CSE_90MSO(obj)
            % Function to get corticospinal excitability
            % # of runs is selected by tms_settings.cse_runs and applied
            % in blocks of # = tms_settings.cse_iterations stimuli
            % EEG channel obj.tms_settings.eeg_fbk_ch is also plotted as
        ...
        function get_corticalmap(obj)
            ...
            pause(1)
            % obj.time = 'pre';
            if exist([obj.directories.map 'map_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.map 'map_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
                    ...
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                obj.objMag.changeAmp(0,0);    % change MagVen to desired stim int
            ...
            save([obj.directories.map 'map_' obj.time], 'obj', '-v7.3')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            disp('Finished')
            obj.objMag.turnOnOff('off');
        end        
        
        function get_corticalmap_step(obj,~)
            % Function to get cortical excitability map
            % # of stimulation points is selected by tms_settings.map_points
            % and one stimulus per location is applied
            obj.check_buffer() %Check if buffer is still recording
...
%                                         pause(5)
        ...
        %% Functions to measure corticospinal excitability in rest and contracted muscle
...
%                                         end
            ...
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'iTMS_' obj.time], 'obj', '-v7.3')
            save([obj.directories.imep 'hotspot_' num2str(rep)], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function iMEP_acquisition(obj)
            ...
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
            % obj.time = 'pre';
            if exist([obj.directories.imep 'imep_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.imep 'imep_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
...
%                                         pause(1)
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'imep_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.changeAmp(0,0)
        end
...
%                                                 obj.plotMEP(obj, data, stimTime, run, rep, h1) 
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'orientation_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function iMEP_ipi(obj)
            ...
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
            % obj.time = 'pre';
            if exist([obj.directories.imep 'pp_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.imep 'pp_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
...
%                                                 obj.plotMEP(obj, data, stimTime, ipi_n, rep, h1)
        ...
        function iMEP_io(obj)
            ...
            obj.objMag.flushData();
            obj.dataEEGEMG = []; %make sure that data matrix is empty
%             obj.time = input('Name of measurement: ','s');
            if exist([obj.directories.imep 'io_' obj.time '.mat'],'file') == 2
                obj.time = input('Name of measurement: ','s');
                if exist([obj.directories.imep 'io_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
                    ...
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                ...
                                h2=subplot(2,1,2);
                                while finish
                                    disp(['Current intensity: ' num2str(obj.tms_settings.io_int(1,int)) '%; Number ' num2str(int) ' of ' num2str(numel(obj.tms_settings.io_int))])
                                    %apply 10 Stimuli for each intensity
                                    for rep = 1:10
                                        if rep ~= 1
                                            pause(1)
...
%                                                 obj.plotMEP(obj, data, stimTime, int, rep, h1)
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'io_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end

        function iMEP_corticalmap(obj, feedback_muscle)
            % Function to get cortical excitability map
            % # of stimulation points is selected by tms_settings.map_points
            % and five stimulus per location is applied
            obj.check_buffer() %Check if buffer is still recording
            obj.objMag.turnOnOff('on');
            obj.objMag.changeAmp(0,0); %change magventure to 0%
            obj.objMag.flushData();
            obj.dataEEGEMG = []; %make sure that data matrix is empty
            obj.time = 'iHS_search';%input('Name of measurement: ','s');
            if exist([obj.directories.imep 'map_' obj.time '.mat'],'file') == 2
                obj.time = input('Name of measurement: ','s');
                if exist([obj.directories.imep 'map_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
            ...
            obj.objMag.flushData();
            nb_rep = 5; %5 stimuli per Stimulation cell point
            finish = 1; %Exit if round>rand_walk_iterations
            round =1; %spot of location in 10x10 grid
            set_back_int = 0;
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
                ...
                if( obj.objMag.spObject.BytesAvailable~=0)
                    switch obj.objMag.spObject.BytesAvailable
                        case 8
                            ...
                            if strcmpi(mag_fbk, 'stimulate') %Manual MagVenture Stimulation
                                while round<=obj.tms_settings.map_points %stays in loop until end of iterations %apply x stimuli at different loations
                                    for rep = 1:nb_rep %5 stimuli per Stimulation cell point
                                        if rep ~= 1
                                            %ISI of 5 +- 1.25 s
                                            while toc(isiTic) < (6.25-3.75)*rand(1)+3.75, end %500ms for MagVen to change int
                                        end
...
%                                         pause(1)
                                        ...
                                        % 3s at X%MVC stimulate
                                        if rep ~= nb_rep
                                            [tmp,isiTic] = obj.stimTMS_act_woLoc(obj,amp);
                                        else
                                            [tmp,isiTic] = obj.stimTMS_act(obj,amp);
                                        end
                                        if(isempty(tmp))
                                        ...
                                        if(~isempty(obj.dataEEGEMG))
                                            ...
                                            if(stimTime > 40 && ~isnan(stimTime))
                                                ...
                                                if rep == nb_rep
                                                    obj.objLoc.sendMEP(nanmean(obj.MEP.peakToPeak(:,round)), nanmean(obj.MEP.iMEP(:,round)), obj.MEP.duration(rep,round));
                                                end
                                            else
                                        ...
                                        while pause_stim
                                            switch obj.objMag.spObject.BytesAvailable
                                                case 8
                                                    mag_fbk = obj.check_mag(obj);
                                                    obj.objMag.flushData();
                                                    if strcmpi(mag_fbk, 'stimulate') %strcmpi(input('Do you want to go to next stimulation point? (y/n)','s'),'n')
                                                        pause_stim = 0;
                                                    end
                                            end
                                        end
...
%                                 break;
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'map_' obj.time], 'obj', '-v7.3')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            disp('Finished')
            obj.objMag.turnOnOff('off');
        end    
        ...
        function iMEP_mvc_range(obj)
            ...
            amp = obj.tms_settings.imep_amp; %Stimulation intensity
            % obj.time = 'pre';
            if exist([obj.directories.imep 'mvc_thresh_range_' obj.time '.mat'],'file') == 2
                obj.time = input('Time: ','s');
                if exist([obj.directories.imep 'mvc_thresh_range_' obj.time '.mat'],'file') == 2
                    obj.time = input('File exists already. Redo Time: ','s');
                end
...
%                                         pause(1)
            ...
            disp('Saving...')
            if ~isdir(obj.directories.imep); mkdir(obj.directories.imep); end
            save([obj.directories.imep 'mvc_thresh_range_' obj.time], 'obj', '-v7.3')
            disp('Saved')
            obj.dataEEGEMG = []; obj.dataEEGEMG_cut = [];
            obj.objMag.turnOnOff('off');
        end
        ...
        function release(obj)
            ...
            obj.PTB.closeScreen;  %close PTB window
    ...
    methods(Static, Access = private)
        ...
        function get_AMT_MEP_inf(obj, data, stimTime, int, rep, thresh)
            % Function to detect MEP during contraction for AMT
            % Possibility to set to 0 if PtP is below thresh
            % Poststim EMG has to be >= 5 ms above SD of prestim EMG
            % Calculate baseline mean and SD of prestim EMG
            base_start = -100; %ms
            base_end   = 0; %ms
            base = data(stimTime+(0.001*base_start*obj.ampSettings.SampRate):stimTime+(0.001*base_end*obj.ampSettings.SampRate));
            base_mean = mean(abs(base));
            base_sd   = std(abs(base));
            base_max  = max(abs(base));
            base_min  = min(abs(base));
            base_ptp  = base_max + abs(base_min);
            % See if datapoint exceeds mean + SD
            x_start = obj.tms_settings.mep_start;    %ms; start of MEP search
            x_end   = obj.tms_settings.mep_end;  %ms; end of MEP search
            ...
            obj.MEP.maxAmp(int,rep)  = max(post);
            obj.MEP.minAmp(int,rep)  = min(post);
            MEP_ptp  = obj.MEP.maxAmp(int,rep) + abs(obj.MEP.minAmp(int,rep));
            obj.MEP.peakToPeak(int,rep)         = (MEP_ptp-base_ptp);
            % MEP threshold gate
            if obj.MEP.peakToPeak(int,rep) < thresh
                obj.MEP.peakToPeak(int,rep) = 0;
                obj.MEP.minAmp(int,rep)     = 0;
            ...
            % If duration is above 5ms consider as MEP 
            if obj.MEP.duration(int,rep) >= 5 %ms
                obj.MEP.isMEP(int,rep) = 1;
            else
                obj.MEP.isMEP(int,rep) = 0;
            end            
        end
        
        function [base, post1] = getiMEPinf(obj, data, stimTime, int, rep)
            ...
            x_start = obj.tms_settings.mep_start;    %ms; start of MEP search
            x_end   = obj.tms_settings.mep_end;  %ms; end of MEP search
            post1 = data(stimTime+(0.001*x_start*obj.ampSettings.SampRate):stimTime+(0.001*x_end*obj.ampSettings.SampRate));
            post = abs(data(stimTime+(0.001*x_start*obj.ampSettings.SampRate):stimTime+(0.001*x_end*obj.ampSettings.SampRate)));
            % Calculate how many seconds the postimulus EMG exceeded the
            % prestimulus mean by > 1 SD
            tmp = find(post> base_mean + 1 * base_sd);
            ...
            if obj.MEP.peakToPeak(int,rep) < 0
                obj.MEP.peakToPeak(int,rep) = 0;
            end
        ...
        function [is_iMEP, ptp]  = get_iMEP_average(obj,data,baseline)
            %Get data
            base = abs(baseline);
            post = abs(data);
            % calculate mean and std of baseline
            base_mean = nanmean(base);
            base_sd   = nanstd(base);
            base_max  = max(base);
            base_min  = min(base);
            base_ptp  = base_max + abs(base_min);
            %% iMEP
            % Calculate how many seconds the postimulus EMG exceeded the prestimulus mean by > 1 SD
            tmp = find(post > (base_mean + 1 * base_sd)); %Find datapoints with values above base_mean + 1 * base_sd
            % Find longest consecutive vector
            tmp_rising = diff(tmp);
            tmp_count = 0; tmp_duration = 0;
            for i = 1:numel(tmp_rising)
            ...
            duration.iMEP = tmp_duration/(obj.ampSettings.SampRate*0.001);
            % If duration is above 5ms consider as iMEP (Ziemann 1999) 
            if duration.iMEP >= 5 %ms
                iMEP_max  = max(data);
                iMEP_min  = min(data);
                iMEP_ptp  = iMEP_max + abs(iMEP_min);
                ptp          = (iMEP_ptp-base_ptp)*duration.iMEP;
                is_iMEP         = 1;
            else
                iMEP_max  = max(data);
                iMEP_min  = min(data);
        ...
        function [finish]=addIntIO(obj)
            ...
            if(strcmp(input('Add intensities? y/n \n', 's'), 'y'))
                new_int = input('Which intensity would you like to repeat [in %MSO] \n');
                obj.tms_settings.io_int        = [obj.tms_settings.io_int new_int];
                finish            = 1;
                obj.pauseFunc(obj, numel(obj.tms_settings.io_int)-1, 1);
            else
                finish            = 0;
            end
        end
        ...
        function [data,isiTic] = stimTMS_rest(obj, varargin) 
            %stimulates with specified intensity if target muscle is at rest
            if nargin == 2
                obj.objMag.changeAmp(varargin{1},0);    % change MagVen to desired stim int
                obj.wait_ms(500)                %wait for mag vent to change amp
            end
            ...
            while 1
                ...
                if size(data_tmp,1) > (0.1*obj.ampSettings.SampRate)
                    ...
                    % only stimulate if EMG is between +-20?V (e.g. Takemi 2013 et al)
                    if max(abs(tmp)) < 20
                        obj.objLPT.write(obj.stim_pin, obj.lpt_ad)                 % stimulus and markers
                        isiTic = tic; %Set timer for ISI
...
%                         keyboard % following scipt has to be checked
        ...
        function [data,isiTic] = stimTMS_act_woLoc(obj, amp)
            %stimulate with specified intensity 
            % while Subject has to contract muscle at certain 
            % threshold to trigger stimulus
            obj.objMag.changeAmp(amp,0); % Change Stim Intensity
            ...
            while 1 %Stimulate if MVC +- range is reached
                if toc(step_tic) >= 0.2 %take last 200ms
                    ...
                    if contr_nb == contr_stim %correct continuous contraction for 2-3s (10-15*200ms)
                        obj.objLPT.write(obj.stim_pin_woLoc, obj.lpt_ad) %stimulus and marker
                        isiTic = tic; %Set timer for ISI
                        break % Exit MVC ratio measurement
                    end
                    step_tic = tic; %reset timer
...
%             obj.MEP.delayMin(int,rep)   = find(abs(data(1:stimTime))== obj.MEP.minAmp(int,rep),1);
    ...
    methods(Static)
        ...
        function  obj = sw_lz_TMS_v4(dataSub, projectPath, ampSettings, tms_settings)
            % Initialisation
            set_path_vars;
            ...
            [a, b] = audioread('GO.mp3');
            obj.audio.contract = audioplayer(a,b);
            [a, b] = audioread('Entspannen.mp3');
        ...
        function ins = getInstance(dataSub, projectPath, ampSettings, tms_settings)
            ...
            if( ~strcmpi(class(instance), 'sw_lz_TMS_v4') )
                instance = sw_lz_TMS_v4(dataSub, projectPath, ampSettings, tms_settings);
            end
